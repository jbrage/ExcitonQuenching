...
def E_MeV_to_beta(E_MeV_per_A, A_projectile):
    '''
    INPUT:
    ...
    - beta = v/c
    '''
    E_MeV = A_projectile * E_MeV_per_A
    E_Joule = E_MeV*1e6*q_electron          # total kinetic energy [Joule]
    m_particle = A_projectile*m_proton        # ion mass [kg]
    beta = np.sqrt(1. - 1./(1. + E_Joule/(m_particle*c**2))**2)
    return beta


def load_ShellCorrection_data(E_MeV_per_A, A_projectile):
    '''
...
def check_BetheBloch_limit(E_MeV_per_A, A_projectile, PRINT_WARNING, Z_A_material = 7.5):
    '''
    Check if the Bethe-Bloch equation is 'applicable',
    Attix chapter 8: Stopping power, A Soft-collision term
    (although) is in good agreement with experimental data for lower Energies_MeV
    ...
    if ratio > 1 or beta > 0.99:
        valid_energy = False
        if PRINT_WARNING:
            msg = "\n# %0.2f MeV/A \n# => (Z/137 beta)**2 = %0.2f" % (E_MeV_per_A, ratio)
            msg += "\n# (Z/137beta)**2 << 1 isn't satisfied\n"
            print(msg)
    return valid_energy


def BetheBlochEq(E_MeV_per_A, z_projectile, A_projectile, Z_A_material = Z_A_water, shell_correction=False, PRINT_WARNING=False):
    '''
    Calculates the Bethe-Bloch stopping power

    INPUT:
    - kinetic energy per nucleon [MeV/A]
    - z_projectile, particle charge, multipla of the elementary charge
    - A_projectile, nucleon number
    - Z_A_material, Z/A of the given material
    - shell_correction (boolean), True loads the shell correction data for liquid water
    - PRINT_WARNING (boolean), True prints a warning of the particle is too slow

    OUTPUT:
    - stopping power in MeV cm^2/g
    - True/False depending on whether the particle energy is too low to a reliable
    calculation of stopping power use Bethe-Bloch
    '''
    valid_energy = check_BetheBloch_limit(E_MeV_per_A, A_projectile, PRINT_WARNING)
    ...
    K =  K1*K2                                  # Joule * m^2
    K *= N_A    # Joule * m^2 /mol => A_water carries units of g/mol => Joule * m^2/g
    ...
    beta = E_MeV_to_beta(E_MeV_per_A, A_projectile)
    gamma = beta_to_gamma(beta)
    Tmax_Joule = beta_to_Tmax_Joule(beta, A_projectile)

    # full equation
    front_factor = K*z_projectile**2 * Z_A_material /beta**2
    term1 = 0.5*np.log(2*m_electron*c**2*beta**2*gamma**2*Tmax_Joule/I_Joule**2)
    term2 = beta**2

    if shell_correction:
        C_Z = load_ShellCorrection_data(E_MeV_per_A, A_projectile)
...
if __name__ == '__main__':
    ...
    print("# z = %i, A = %i" % (z_projectile, A_projectile))

    txt = "# E [MeV/A], \tdEdx [MeV cm^2/g] \tEq. applicable?"
    print(txt)
    for E_MeV_per_A in Energies_MeV:
        dEdx_MeV_cm2_g, applicable_energy = BetheBlochEq(E_MeV, z_projectile, A_projectile, Z_A_water)
        print("%0.2f,\t\t%0.2f\t\t\t%s" % (E_MeV/A_projectile, dEdx_MeV_cm2_g, applicable_energy))
